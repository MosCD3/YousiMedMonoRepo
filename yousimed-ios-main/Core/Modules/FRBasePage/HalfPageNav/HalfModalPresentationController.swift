...
//        blurEffectView.contentView.addSubview(vibrancyEffectView)
    ...
    override var frameOfPresentedViewInContainerView: CGRect {
        let menuHeightPercent =  0.75
        let menuPlamentY = 1 - menuHeightPercent
        
        let fullHeight = containerView!.bounds.height
        let menuPlacment = fullHeight * CGFloat(menuPlamentY)
        let menuHeight = fullHeight * CGFloat(menuHeightPercent) 
        return CGRect(x: 0, y: menuPlacment, width: containerView!.bounds.width, height: menuHeight )
    }
